...
async function runHttpE2eTest() {
  console.log("🚀 Starting standalone Next.js server for HTTP E2E verification...");
  ...
  try {
    ...
    const templatesHtml = await templatesPageRes.text();
    if (!templatesHtml.includes("Classic") || !templatesHtml.includes("Modern")) {
      throw new Error("/templates missing template cards");
    }
    console.log("  ✓ /templates page renders the 5 book personality cards.");
    ...
    const settingsHtml = await settingsPageRes.text();
    if (!settingsHtml.includes("Format &amp; Print Specifications") && !settingsHtml.includes("Format & Print Specifications")) {
      throw new Error("/settings missing header");
    }
    console.log("  ✓ /settings page renders with Simple Mode and Advanced controls.");
    ...
    console.log("  ✓ /create page renders checklist status screen.");
    ...
    console.log("  ✓ /ready page renders download and email actions.");
